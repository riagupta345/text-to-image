...
def S2_build_embedding_compressor_model():
    """
...
def S2_build_stage1_generator():
    ...
    c = Lambda(S2_generate_c)(mean_logsigma)
...
def S2_build_stage2_generator():
    ...
    # 3. Joint
    c_code = Lambda(S2_joint_block)([c, x])
...
def S2_build_stage2_discriminator():
    """
...
def S2_build_adversarial_model(gen_model2, dis_model, gen_model1):
    """
...
def S2_load_class_ids(class_info_file_path):
    """
...
def S2_load_embeddings(embeddings_file_path):
    """
...
def S2_load_filenames(filenames_file_path):
    """
...
def S2_load_bounding_boxes(dataset_dir):
    """
...
def S2_get_img(img_path, bbox, image_size):
    """
...
def S2_load_dataset(filenames_file_path, class_info_file_path, cub_dataset_dir, embeddings_file_path, image_size):
    filenames = S2_load_filenames(filenames_file_path)
    class_ids = S2_load_class_ids(class_info_file_path)
    bounding_boxes = S2_load_bounding_boxes(cub_dataset_dir)
    all_embeddings = S2_load_embeddings(embeddings_file_path)
...
#    print("All embeddings shape:", all_embeddings.shape)
    ...
    for index, filename in enumerate(filenames):
        ...
        try:
            ...
            img = S2_get_img(img_name, bounding_box, image_size)
...
def S2_custom_generator_loss(y_true, y_pred):
    # Calculate binary cross entropy loss
    return K.binary_crossentropy(y_true, y_pred)


def S2_write_log(callback, name, loss, batch_no):
    """
...
Load datasets
"""
X_hr_train, y_hr_train, embeddings_train = S2_load_dataset(filenames_file_path=filenames_file_path_train,
                                                        class_info_file_path=class_info_file_path_train,
                                                        cub_dataset_dir=cub_dataset_dir,
                                                        embeddings_file_path=embeddings_file_path_train,
                                                        image_size=(256, 256))

X_hr_test, y_hr_test, embeddings_test = S2_load_dataset(filenames_file_path=filenames_file_path_test,
                                                     class_info_file_path=class_info_file_path_test,
                                                     cub_dataset_dir=cub_dataset_dir,
                                                     embeddings_file_path=embeddings_file_path_test,
                                                     image_size=(256, 256))

X_lr_train, y_lr_train, _ = S2_load_dataset(filenames_file_path=filenames_file_path_train,
                                         class_info_file_path=class_info_file_path_train,
                                         cub_dataset_dir=cub_dataset_dir,
                                         embeddings_file_path=embeddings_file_path_train,
                                         image_size=(64, 64))

X_lr_test, y_lr_test, _ = S2_load_dataset(filenames_file_path=filenames_file_path_test,
                                       class_info_file_path=class_info_file_path_test,
                                       cub_dataset_dir=cub_dataset_dir,
                                       embeddings_file_path=embeddings_file_path_test,
                                       image_size=(64, 64))

#print(X_hr_train.shape, y_hr_train.shape, embeddings_train.shape)
#print(X_hr_test.shape, y_hr_test.shape, embeddings_test.shape)
...
all_gen_loss = []
all_disc_loss = []
dir_results =dir_to_change+"Output_Folder/model2_stage2_results/"
stage2_dis = S2_build_stage2_discriminator()
stage2_dis.compile(loss='binary_crossentropy', optimizer=dis_optimizer)

stage1_gen = S2_build_stage1_generator()
stage1_gen.compile(loss="binary_crossentropy", optimizer=gen_optimizer)

dir_weights = dir_to_change+"Output_Folder/"
stage1_gen.load_weights(dir_weights+"stage1_gen_wgan.h5")

stage2_gen = S2_build_stage2_generator()
stage2_gen.compile(loss="binary_crossentropy", optimizer=gen_optimizer)

embedding_compressor_model = S2_build_embedding_compressor_model()
embedding_compressor_model.compile(loss='binary_crossentropy', optimizer='adam')

adversarial_model = S2_build_adversarial_model(stage2_gen, stage2_dis, stage1_gen)
adversarial_model.compile(loss=['binary_crossentropy', S2_KL_loss], loss_weights=[1.0, 2.0],
                          optimizer=gen_optimizer, metrics=None)

# log_folder = dir_to_change + "Output_Folder/stage2_logs/"

# tensorboard = TensorBoard(log_dir=log_folder.format(time.time()))
...
#        print("g_loss:{}".format(g_loss))
    ...
    """
#     S2_write_log(tensorboard, 'discriminator_loss', np.mean(dis_losses), epoch)
#     S2_write_log(tensorboard, 'generator_loss', np.mean(gen_losses)[0], epoch)

    all_gen_loss.append(np.mean(gen_losses)[0])
    all_disc_loss.append(np.mean(dis_losses))
    ...
    if epoch % 2 == 0:
        ...
        for i, img in enumerate(hr_fake_images[:10]):
            S2_save_rgb_img(img, (dir_results+"gen_{}_{}.jpg".format(epoch, i)))

# Saving the models
dir_weights = dir_to_change+"Output_Folder/"
stage2_gen.save_weights(dir_weights+"model2_stage2_gen.h5")
stage2_dis.save_weights(dir_weights+"model2_stage2_dis.h5")


# In[ ]:


epoch_count = range(1, len(all_disc_loss) + 1)
plt.plot(epoch_count, all_disc_loss, 'r', label="Discriminator Loss")
plt.xlabel('Epochs')
plt.ylabel('Loss')
# plt.legend(['Discriminator Loss'])
plt.legend()
plt.title("Stage 2: Discriminator Loss")
# plt.show()
dir_plot = dir_to_change + "Output_Folder/"
plt.savefig(dir_plot+"Stage2_DiscriminatorLoss")


# In[ ]:


epoch_count = range(1, len(all_gen_loss) + 1)
plt.plot(epoch_count, all_gen_loss, 'b', label="Generator Loss")
plt.xlabel('Epochs')
plt.ylabel('Loss')
# plt.legend(['Generator Loss'])
plt.legend()
plt.title("Stage 2: Generator Loss")
# plt.show()
dir_plot = dir_to_change + "Output_Folder/"
plt.savefig(dir_plot+"Stage2_GeneratorLoss")

df = pd.DataFrame(data={"col1": all_disc_loss, "col2": all_gen_loss})
df.to_csv("./Model 2 stage2 losses.csv", sep=',',index=False)
